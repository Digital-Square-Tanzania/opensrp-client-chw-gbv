...
import static org.smartregister.chw.gbv.util.Constants.EVENT_TYPE.DELETE_EVENT;
import static org.smartregister.chw.gbv.util.Constants.JSON_FORM_EXTRA.DELETE_EVENT_ID;
import static org.smartregister.chw.gbv.util.Constants.JSON_FORM_EXTRA.DELETE_FORM_SUBMISSION_ID;
import static org.smartregister.chw.gbv.util.JsonFormUtils.HOME_VISIT_GROUP;
...
public class VisitUtils {
    ...
    public static List<Visit> getVisitsOnly(String memberID, String visitName) {
        return new ArrayList<>(GbvLibrary.getInstance().visitRepository().getVisits(memberID, visitName));
    }

    public static List<VisitDetail> getVisitDetailsOnly(String visitID) {
        return GbvLibrary.getInstance().visitDetailsRepository().getVisits(visitID);
    }
    ...
    public static void processVisits(String baseEntityID) throws Exception {
        processVisits(GbvLibrary.getInstance().visitRepository(), GbvLibrary.getInstance().visitDetailsRepository(), baseEntityID);
    }
    ...
    public static void deleteProcessedVisit(String visitID, String baseEntityId) {
        // check if the event
        AllSharedPreferences allSharedPreferences = GbvLibrary.getInstance().context().allSharedPreferences();
        Visit visit = GbvLibrary.getInstance().visitRepository().getVisitByVisitId(visitID);
        if (visit == null || !visit.getProcessed()) return;

        Event processedEvent = GbvDao.getEventByFormSubmissionId(visit.getFormSubmissionId());
