...
public class GbvUtil {
    ...
    public static void processEvent(AllSharedPreferences allSharedPreferences, Event baseEvent) throws Exception {
        if (baseEvent != null) {
            GbvJsonFormUtils.tagEvent(allSharedPreferences, baseEvent);
            JSONObject eventJson = new JSONObject(GbvJsonFormUtils.gson.toJson(baseEvent));
    ...
    public static boolean launchDialer(final Activity activity, final BaseGbvCallDialogContract.View callView, final String phoneNumber) {
        if (ContextCompat.checkSelfPermission(activity, Manifest.permission.READ_PHONE_STATE) != PackageManager.PERMISSION_GRANTED) {

            // set a pending call execution request
            if (callView != null) {
                callView.setPendingCallRequest(() -> GbvUtil.launchDialer(activity, callView, phoneNumber));
            }
    ...
    public static void saveFormEvent(final String jsonString) throws Exception {
        ...
        Event baseEvent = GbvJsonFormUtils.processJsonForm(allSharedPreferences, jsonString);
        GbvUtil.processEvent(allSharedPreferences, baseEvent);
    }
